...
int main( int argc, char* argv[] ) {
 ...
 const Event* ev;
  ...
  while ( ( ev = read(file) ) != 0 ) {
    ...
    ba.compute();
    const vector<float> baM=ba.eMean();
    const vector<float> baR=ba.eRms();
   ...
   af.compute();
   const vector<float> afM=af.eMean();
   const vector<float> afR=af.eRms();
   ...
   as.compute();
   const vector<float> asM=as.eMean();
   const vector<float> asR=as.eRms();
  ...
  au.compute();
  const vector<float> auM=au.eMean();
  const vector<float> auR=au.eRms();
